...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/06 15:50:09 by mtavares          #+#    #+#             */
/*   Updated: 2023/11/06 15:50:22 by mtavares         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int Span::shortestSpan() {
  ...
  std::sort(_numbers.begin(), _numbers.end());
  res = longestSpan();
  for (std::vector<int>::iterator it = _numbers.begin();
       it != _numbers.end() - 1; it++) {
    if (res > *(it + 1) - *it)
      res = *(it + 1) - *it;
  }
  return res;
...
const char *Span::LowNumbers::what() const throw() {
  return "Does not have enough size";
}

const char *Span::SizeTooHigh::what() const throw() {
  return "Can not add another number";
}
